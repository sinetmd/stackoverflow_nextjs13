...
const GlobalResult = () => {
  ...
  // add results based of search
  const [result, setResult] = useState([
    {
      type: "question",
      id: 1,
      title: "NextJSQuestion",
    },
    {
      type: "tag",
      id: 3,
  ...
  // get the filter (what type of data: question/answer/tag etc.)
  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      try {
        // call the db at once from everywhere
  ...
  return (
    <div className="absolute top-full z-10 mt-3 w-full rounded-xl bg-light-800 py-5 shadow-sm dark:bg-dark-400">
      <GlobalFilters />
      {/* divider line between found search list items */}
              ...
              result.map((item: any, index: number) => (
                <Link
                  ...
                  className="flex w-full cursor-pointer items-start 
                    gap-3 px-5 py-2.5 hover:bg-light-700/50 dark:bg-dark-500/50"
                >
                  <Image
